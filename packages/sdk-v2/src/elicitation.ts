...
/* Ported from v1 unchanged. A tool that needs user input mid-call suspends and
 * the host's `onElicitation` handler (executor-level, overridable per `execute`)
 * answers. Tools that never elicit never trigger it. */
