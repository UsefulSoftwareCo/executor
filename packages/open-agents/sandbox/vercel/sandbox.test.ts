...
function createMockSandboxSdk(name: string) {
  ...
  return {
    ...
    update: async (params: { networkPolicy?: unknown }) => {
      updateNetworkPolicyCalls.push(params.networkPolicy);
    },
    writeFiles: async (files: { path: string; content: Buffer }[]) => {
      writeFilesCalls.push(files);
    },
...
describe("VercelSandbox.create", () => {
  ...
  test("creates from base snapshot and clones multiple git sources", async () => {
    await sandboxModule.VercelSandbox.create({
      baseSnapshotId: "snap-base-1",
      sources: [
        {
    ...
    expect(runCommandCalls.filter((call) => call.cmd === "git")).toEqual([
      {
        cmd: "git",
        args: [
          "clone",
      ...
      {
        cmd: "git",
        args: ["checkout", "-b", "linear/voi-123"],
        cwd: "/vercel/sandbox/augment-services",
      },
    ]);
  });
