/* oxlint-disable executor/no-try-catch-or-throw, executor/no-error-constructor, executor/no-promise-catch -- boundary: sandbox git helpers expose a Promise shell adapter and must restore worktrees around git command failures */
...
function exec(sandbox: Sandbox, command: string, timeoutMs = 30000): Promise<ExecResult> {
...
async function fetchRemoteBranch(sandbox: Sandbox, branch: string): Promise<"fetched" | "missing"> {
  ...
  throw new Error(`Failed to fetch remote branch: ${commandOutput(fetchResult)}`);
}

async function resetToFetchedRemoteBranch(sandbox: Sandbox, branch: string): Promise<void> {
  const resetResult = await exec(sandbox, `git reset --hard origin/${branch}`, 10000);
  if (!resetResult.success) {
    throw new Error(`Failed to reset to remote branch: ${commandOutput(resetResult)}`);
  }
  ...
  if (!upstreamResult.success) {
    throw new Error(`Failed to set upstream after remote sync: ${commandOutput(upstreamResult)}`);
  }
}
...
async function getCurrentHead(sandbox: Sandbox): Promise<string> {
  ...
  if (!headResult.success) {
    throw new Error(`Failed to inspect current HEAD: ${commandOutput(headResult)}`);
  }
...
async function resetToCommit(sandbox: Sandbox, commit: string): Promise<void> {
  ...
  if (!cleanResult.success) {
    throw new Error(`Failed to clean worktree after sync failure: ${commandOutput(cleanResult)}`);
  }
}
 ...
 */
export async function hasUncommittedChanges(sandbox: Sandbox): Promise<boolean> {
...
export async function getChangedFiles(sandbox: Sandbox): Promise<FileChange[]> {
  const result = await exec(sandbox, "git diff --cached --name-status -z HEAD", 15000);
 ...
 */
export async function detectBinaryFiles(sandbox: Sandbox): Promise<Set<string>> {
  const result = await exec(sandbox, "git diff --cached --numstat -z HEAD", 15000);
 ...
 */
export async function getFileModes(sandbox: Sandbox): Promise<Map<string, string>> {
 ...
 */
export async function syncToRemote(sandbox: Sandbox, branch: string): Promise<void> {
...
export async function syncToRemotePreservingChanges(
  ...
  if (!statusResult.success) {
    throw new Error(`Failed to inspect local changes: ${commandOutput(statusResult)}`);
  }
  ...
  if (!stashResult.success) {
    throw new Error(`Failed to stash local changes: ${commandOutput(stashResult)}`);
  }
