-- fumadb-effect deviation: timestamp is datetime(3) (MySQL timestamp has second precision, a 1970-2038 range, and depends on the session time zone).
-- fumadb-effect deviation: a `decimal` column is created as `decimal(65,30)`.
-- A bare `decimal` is DECIMAL(10, 0) on MySQL, which rounds 1.5 to 2.
SET FOREIGN_KEY_CHECKS = 0;

create table `prefix_0_users` (`id` varchar(255) not null primary key, `image` varchar(200) default 'my-avatar', `data` longblob);
...
alter table `prefix_1_users` add column `decimal` decimal(65,30);

alter table `prefix_1_users` add column `bool` boolean;

alter table `prefix_1_users` add column `json` json;

alter table `prefix_1_users` add column `binary` longblob;

alter table `prefix_1_users` add column `date` date;

alter table `prefix_1_users` add column `timestamp` datetime(3);
