/** Upgrade real version 3 rows through the same storage migration used by all products. */
...
import { storageSchemaV3 } from "../src/implementation/storage-schema-v3.ts";

const oldDatabase = fumadb({ namespace: "executor", schemas: [storageSchemaV3] });
const fixture = Effect.gen(function* () {
  ...
  const seed = `
    ...
    INSERT INTO executor_apps (id, code, repository, owner, name, active_deployment, accounts, copied_from, created_at, slug)
...
const snapshot = Effect.gen(function* () {
  const sql = yield* SqlClient.SqlClient;
  const tables = [
    ...Object.values(storageSchemaV3.tables).map((table) => table.names.sql),
    "host_owned_fixture",
  ];
  return yield* Effect.forEach(
    tables,
    (table) =>
      sql`SELECT to_jsonb(record) - CASE WHEN ${table} = 'executor_apps' THEN 'accounts' ELSE '' END AS value
...
test("version 3 upgrade preserves profiles, credentials, app data and background work", () =>
  run(
    Effect.gen(function* () {
      ...
      assert.equal(
        (yield* sql`SELECT 1 FROM information_schema.columns WHERE table_name='executor_apps' AND column_name='accounts'`)
          .length,
        0,
      );
      ...
      assert.equal(apps[0]?.name, "Fixture");
      assert.equal(apps[0]?.deployment?.id, "dpl_fixture");
      assert.equal(Object.hasOwn(apps[0] ?? {}, "accounts"), false);
      yield* storage.migrate;
      assert.deepEqual(yield* snapshot, before);
      assert.equal(
        (yield* sql`SELECT indexname FROM pg_indexes WHERE indexname IN ('executor_workflow_runs_context_key','executor_webhooks_context_key','executor_schedules_context_name')`)
          .length,
        3,
      );
    }),
...
for (const [description, statement] of [
  ["app selections", `UPDATE executor_apps SET accounts='{"service":"acc_fixture"}'`],
  ["unprofiled running workflows", "UPDATE executor_workflow_runs SET installation=null"],
  ["unprofiled webhooks", "UPDATE executor_webhooks SET installation=null"],
  ["unprofiled schedules", "UPDATE executor_schedules SET installation=null"],
  ["unprofiled pending scheduled runs", "UPDATE executor_scheduled_runs SET installation=null"],
  [
    "unprofiled connection targets",
    `UPDATE executor_account_connections SET target='{"app":"app_fixture","requirement":"service"}'`,
  ],
] as const) {
  test(`upgrade refuses ${description} without changing version or data`, () =>
    run(
      Effect.gen(function* () {
        const { sql, old } = yield* fixture;
        yield* sql.unsafe(statement).unprepared;
        const before = yield* snapshot;
        const selections = yield* sql`SELECT accounts::jsonb AS accounts FROM executor_apps`;
        const storage = yield* makeExecutorStorage({ provider: "postgresql" });
        const result = yield* Effect.result(storage.migrate);
        assert.equal(Result.isFailure(result), true);
        assert.deepEqual(yield* (yield* old.createMigrator).version, Option.some("3.0.0"));
        assert.deepEqual(yield* snapshot, before);
        assert.deepEqual(
          yield* sql`SELECT accounts::jsonb AS accounts FROM executor_apps`,
          selections,
        );
      }),
    ));
...
test("upgrade retains unprofiled terminal history and account-free background work", () =>
  run(
    Effect.gen(function* () {
      const { sql, old } = yield* fixture;
      yield* sql`UPDATE executor_deployments SET requirements='{"accounts":{}}'`;
      yield* sql`UPDATE executor_workflow_runs SET installation=null, status='complete'`;
      yield* sql`UPDATE executor_schedules SET installation=null`;
      yield* sql`UPDATE executor_scheduled_runs SET installation=null`;
      yield* sql`UPDATE executor_account_connections SET target=null`;
      const before = yield* snapshot;
      const storage = yield* makeExecutorStorage({ provider: "postgresql" });
      yield* storage.migrate;
      assert.deepEqual(yield* snapshot, before);
...
test("a late migration failure rolls back the column removal and can be retried", () =>
  run(
    Effect.gen(function* () {
      const { sql, old } = yield* fixture;
      const before = yield* snapshot;
      ...
      assert.deepEqual(yield* (yield* old.createMigrator).version, Option.some("3.0.0"));
      assert.deepEqual(yield* sql`SELECT accounts::jsonb AS accounts FROM executor_apps`, [
        { accounts: {} },
      ]);
      assert.deepEqual(yield* snapshot, before);
