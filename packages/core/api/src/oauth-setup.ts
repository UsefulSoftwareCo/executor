...
// Kept independent of the React app, like the OAuth callback document: this
// page also opens from MCP links and hosts that only serve the HTTP API.
const setupStyles = `
  :root {
...
const renderDocument = (title: string, content: string): string => `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  return renderDocument(
    ...
    `
      ...
      <section aria-labelledby="authorize-heading">
        ...
        <a class="primary" href="${escapeHtml(continuation)}" rel="noreferrer">
...
export const oauthSetupUnavailableDocument = (): string =>
  renderDocument(
    ...
    `
      <h1>Connection setup unavailable</h1>
      <p>
        This setup link has expired or is no longer available.
        Close this window and start connecting again.
      </p>
    `,
  );
