...
import { firstPartyOAuthClientSlug, type TokenEndpointAuthMethod } from "./oauth-client";
              ...
              refreshGrant(input: RefreshGrantInput) {
                ...
                return Effect.suspend(() => {
                  if (behaviour.kind === "rejected") {
                    const rejection = new RefreshGrantRejected(
...
describe("provider-owned OAuth refresh grant", () => {
  ...
  const scenario = (options: {
    ...
    /** Back the connection with a config-declared app whose secret is host config, not a
     *  provider item. */
    ...
    Effect.gen(function* () {
      ...
      const server = yield* serveOAuthTestServer({
        scopes: ["read"],
        ...(options.tokenEndpointAuthMethod === "basic"
          ? { defaultTokenEndpointAuthMethod: "client_secret_basic" as const }
          : {}),
      });
      const plugins = [
      ...
      const started = yield* executor.oauth.start({
        owner: "org",
        client: options.firstParty === true ? FIRST_PARTY : CLIENT,
        clientOwner: "org",
  ...
  /** Refresh-token grants the HOST posted to the authorization server. Zero of them is the other
   *  half of the custody claim: not only was the token never resolved, it was never spent here. */
  const hostRefreshRequests = (server: {
    readonly requests: Effect.Effect<readonly { readonly path: string; readonly body: string }[]>;
  }) =>
  ...
  it.effect("delegates the grant and never resolves the refresh token through the host", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        // And the host never spent the grant itself: no refresh-token request reached the
        // authorization server from this process.
        expect(yield* hostRefreshRequests(server)).toHaveLength(0);
  ...
  it.effect("falls back to the host-side exchange when the provider cannot do the grant", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const { recorder, server, executor } = yield* scenario({ behaviour: null });
        ...
        expect(yield* hostRefreshRequests(server)).toHaveLength(1);
      }),
    ),
  );

  it.effect("hands the provider the client auth method recorded on the app", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        // The host records a negotiated method per app and presents the secret that way on its own
        // exchange. Telling the provider "body" regardless would make a delegated refresh fail on
        // every client_secret_basic app — a guess where a recorded answer exists.
        expect(recorder.grants).toHaveLength(1);
        expect(recorder.grants[0]?.clientAuth).toBe("basic");
      }),
    ),
  );
  ...
  it.effect("surfaces a refused grant as re-auth and arms the known-dead gate", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        expect(row?.last_health).toMatchObject({
          status: "expired",
  ...
  it.effect("leaves an app whose secret is host config on the host-side exchange", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const { recorder, server, executor } = yield* scenario({
          behaviour: SEALS,
          firstParty: true,
        });
        yield* executor.execute(TOOL, {});

        // A config-declared app's secret is a literal in this process, not a provider item, so
        // there is no id to name it by. Delegating anyway would hand the provider a grant it
        // cannot authenticate, and the authorization server would refuse every refresh.
        expect(recorder.grants).toHaveLength(0);
        expect(yield* hostRefreshRequests(server)).toHaveLength(1);
      }),
    ),
