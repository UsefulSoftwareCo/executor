...
const stripKnownApiMount = (pathname: string): string => {
  const path = stripTrailingSlash(pathname);
  if (path === "/api") return "/";
  if (path.endsWith("/api/executor")) {
    return path.slice(0, -"/api/executor".length) || "/";
  }
  return path || "/";
};
...
export const normalizeExecutorServerOrigin = (raw: string): string => {
  ...
  parsed.pathname = stripKnownApiMount(parsed.pathname);
  parsed.search = "";
  parsed.hash = "";
