...
import type { TokenEndpointAuthMethod } from "./oauth-client";
...
export interface RefreshGrantInput {
  ...
  /** How to present the client secret, as recorded on the OAuth app: `"body"`
   *  is `client_secret_post`, `"basic"` is `client_secret_basic`, and
   *  `"basic_raw"` is HTTP Basic without form-encoding the two halves, for
   *  authorization servers that reject the encoded form. Passed explicitly so a
   *  provider never has to guess — RFC 6749 §2.3.1 prefers Basic while this
   *  caller's default is post, so a guess would be wrong as often as right. */
  readonly clientAuth: TokenEndpointAuthMethod;
  /** How to encode the request body: `"form"` is the RFC 6749 §4.1.3
   *  `application/x-www-form-urlencoded` default, `"json"` is for the
   *  authorization servers that accept only JSON. Absent means `"form"`. */
 ...
 *  token-endpoint refusal is the AS's standing verdict — `invalid_grant` in
 *  particular means the refresh token is dead and only re-authentication
 *  recovers it. Without this the caller cannot tell "the vault is down" from
 *  "this connection is finished", so it can neither prompt for re-auth nor stop
 *  re-sending a grant that will never succeed.
 *
 *  Only the closed standards-defined classification crosses this boundary. In
 *  particular there is deliberately no provider-controlled message or cause:
 *  those fields can contain response bodies, URLs, or secret-bearing errors and
 *  would be surfaced to callers, persistence, or logs by the host. */
