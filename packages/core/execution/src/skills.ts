...
const EXECUTE_SKILL_BODY = [
  ...
  "- `data` is the upstream payload itself — for MCP-backed tools too: the CallToolResult envelope is unwrapped, so `data` is the structured payload (or the parsed JSON a server sent as text, or the text itself, or the ordered content-block array for media/multi-part results). Never look for `data.content[0].text`.",
  "- HTTP-backed tools (OpenAPI) also set `http: { status, headers }` beside `data` — read `result.http?.headers` for pagination (Link) or rate-limit headers. MCP-backed tools may set `content` (supplemental rich blocks such as images — pass them to `emit(...)` to show the user) and `meta` beside `data`.",
