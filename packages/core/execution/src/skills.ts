...
const EXECUTE_SKILL_BODY = [
  ...
  '2. Inspect `matches[0].kind`: `"tool"` results have a callable `path`; `"integration"` results identify an available integration that may need a connection.',
  ...
  "## Rules",
  "",
  '- `tools.search()` returns connected tools (`kind: "tool"`) and available integrations (`kind: "integration"`), including integrations with no connection. Use short intent phrases like `github issues`, `repo details`, or `create calendar event`.',
  ...
  '- Always use the full address when calling tools: `tools.<integration>.<owner>.<connection>.<tool>(args)`. A `kind: "tool"` search result contains the exact callable `path`; a `kind: "integration"` result is discovery metadata and has no callable path.',
