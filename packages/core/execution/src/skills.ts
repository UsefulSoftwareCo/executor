...
const EXECUTE_SKILL_BODY = [
  ...
  "4. Use `details.inputTypeScript` / `details.outputTypeScript` and `details.typeScriptDefinitions` for compact shapes. Check `details.inputConstraints` for validation rules TypeScript cannot express, such as numeric, collection, and string-length limits.",
  ...
  "- MCP-backed tools return their successful `CallToolResult` in `result.data`. Executor converts MCP `isError: true` responses into outer `{ ok: false }`, but a server may also encode application status inside `data.structuredContent` (for example `ok`, `status`, or `error`). Follow the described output shape and check those domain fields before treating the operation as successful.",
...
const CREATE_ARTIFACT_SKILL_BODY = [
  ...
  "the companion `skills` MCP tool with name `artifact-style`. Artifacts render inside the Executor console,",
  ...
  "- Lay the artifact out as an APP, not a document: root `flex h-full flex-col`, headers and filters as ordinary children, and the one long list or table as `flex-1 min-h-0 overflow-auto` so it scrolls under a header that stays put. See the `artifact-style` guide from the companion `skills` MCP tool.",
  ...
  "- Style with the design system, not by taste: tokens only (`text-muted-foreground`, `border-border`), never a hex literal or a Tailwind palette class, and never a page-level `p-6` — the shell owns the outer padding. See the `artifact-style` guide from the companion `skills` MCP tool.",
