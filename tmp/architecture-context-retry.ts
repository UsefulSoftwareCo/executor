/**
 * Retry of the four scouts that failed the burst-spawn race.
 * Staggers spawns ~8s apart to avoid simultaneous worker-unit launches.
 *
 *   bun /home/dana/projects/pi/agent/extensions/agent-sessions/program/conductor/runner.ts ./tmp/architecture-context-retry.ts
 */
...
const SCOUTS: ScoutSpec[] = [
  ...
  },
  {
    ...
    task: [
      ...
      "3) Yjs persistence patterns with Postgres (storing updates + snapshots).",
    ...
    instructions: [
      ...
      `Repo root for node_modules: ${REPO}.`,
...
export default async function contextFleetRetry(c: Conductor): Promise<unknown> {
  const results: Record<string, unknown> = {};

  await Promise.all(
    SCOUTS.map(async (spec, i) => {
      await sleep(i * 8_000);
      try {
        ...
        results[spec.key] = { briefingPath: asset.briefingPath, pins: asset.pins, source: asset.sourceSessionId, chars: asset.briefing.length };
