...
**Stop using this account** removes the profile's selection. It keeps the saved account
...
`profiles_create`; local uses `appProfiles_create`. Read their discovered schemas
for the required fields.

Supply either `target`, to fill a specific profile requirement, or `provider`, to save
an account without selecting it anywhere. Not both.

Check whether you finished:

```js
const connection = await tools.executor.profiles["<management-profile-id>"].queries.accountConnections_get({
