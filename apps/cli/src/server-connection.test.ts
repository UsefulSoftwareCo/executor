...
describe("CLI server connection", () => {
  ...
  it("preserves embedded executor API mounts", () => {
    const connection = parseCliExecutorServerConnection("http://localhost:3000/api/executor", {});

    expect(connection.origin).toBe("http://localhost:3000");
    expect(connection.apiBaseUrl).toBe("http://localhost:3000/api/executor");
    expect(canAutoStartCliServerConnection(connection)).toBe(false);
  });
