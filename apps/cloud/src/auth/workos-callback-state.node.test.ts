...
const stubUsers = Layer.succeed(UserStoreService)({
  use: (_op, fn) =>
    Effect.promise(() =>
      fn({
        ensureAccount: async (id: string) => ({ id }),
        getAccount: async (id: string) => ({ id }),
        upsertOrganization: async (org: { id: string; name: string }) => ({
          ...org,
          slug: org.id,
        }),
        getOrganization: async (id: string) => ({ id, name: `Org ${id}`, slug: id }),
        getOrganizationBySlug: async (slug: string) => ({ id: slug, name: slug, slug }),
...
describe("workos callback · CSRF state hardening", () => {
  it("rejects a callback with NO state (the former bypass) before any WorkOS call", async () => {
    const res = await run(
      new Request(callbackUrl(undefined), { redirect: "manual" }),
    );
