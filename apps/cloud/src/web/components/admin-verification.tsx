...
/** Require a second factor before mounting cloud admin controls. Members retain their existing view. */
export function AdminVerification({
  ...
  return (
    <VerificationFlow key={scope} notice={notice}>
      {children}
    </VerificationFlow>
  );
}
...
function VerificationFlow({
  ...
  if (notice) {
    if (!status || status.state === "member") return null;
    return (
      <div className="border-b border-border px-6 py-2 text-sm text-muted-foreground">
        {status.state === "verified" ? (
          <>
            Administration is unlocked for this session.{" "}
            <Button
              type="button"
              variant="link"
          ...
          <>
            <Link to="/{-$orgSlug}/org" params={(previous) => previous} className="underline">
              Unlock administration
            </Link>{" "}
            to change workspace settings. Personal connections and tools remain available.
          </>
        )}
      </div>
    );
  }
  if (status?.state === "member" || status?.state === "verified") return children;
  ...
  return (
    ...
    >
      <h2 className="text-lg font-medium">Unlock administration</h2>
      <p className="mt-2 text-sm text-muted-foreground">
        Use an authenticator app to unlock admin settings for 15 minutes. You can keep using
        personal connections and tools without setting this up.
      </p>
      <Link
        to="/{-$orgSlug}"
        ...
        >
          ...
          {challenge.kind === "challenge" && (
            <p className="text-sm text-muted-foreground">
              ...
              <a
                className="underline"
                href="mailto:rhys@executor.sh?subject=Admin%20access%20recovery"
              >
                Contact support
              </a>
              .
