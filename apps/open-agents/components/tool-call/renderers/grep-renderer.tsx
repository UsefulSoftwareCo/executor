...
function getGrepContent(output: unknown): string | undefined {
  if (typeof output !== "object" || output === null) return undefined;
  return "content" in output && typeof output.content === "string"
    ? output.content
    : undefined;
}
...
export function GrepRenderer({
  ...
  const content = getGrepContent(output);
  const uniqueFiles = getUniqueFiles(matches);
  const matchCount =
    output && "matchCount" in output && typeof output.matchCount === "number"
      ? output.matchCount
      : matches.length;
  const outputError =
    output && "error" in output ? (output.error ?? "Grep failed") : undefined;
  ...
  return (
    <ToolLayout
      ...
      meta={output ? `${matchCount} matches` : undefined}
      state={mergedState}
