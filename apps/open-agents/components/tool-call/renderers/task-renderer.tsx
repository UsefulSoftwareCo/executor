...
import type { TaskPendingToolCall } from "@/lib/chat/tool-contracts";
...
function getToolMeta(toolName: string): ToolMeta {
  switch (toolName) {
    ...
    case "bash":
      return {
        displayName: "Shell",
        icon: <Hammer className={TOOL_ICON_CLASS} />,
      };
    case "grep":
      return {
        displayName: "Grep",
...
export function TaskRenderer({
  ...
  onApprove,
  onDeny,
}: ToolRendererProps<"tool-agent">) {
  const input = part.input;
  const desc = input?.message ?? "Spawning subagent";
  const subagentType = undefined;
