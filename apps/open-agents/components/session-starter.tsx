...
export function SessionStarter({ onSubmit, isLoading, lastRepo }: SessionStarterProps) {
  ...
  const [vercelProjectChoice, setVercelProjectChoice] = useState<string | null | undefined>(
    undefined,
  );
  const [selectedAgentValue, setSelectedAgentValue] = useState("__default__");

  const { session, loading: sessionLoading, hasGitHub, hasGitHubCli } = useSession();
  const isTrialUser = session?.isManagedTemplateTrialUser ?? false;
  const { reconnectRequired, isLoading: githubConnectionLoading } = useGitHubConnectionStatus({
    enabled: hasGitHub,
  });
  const { preferences, loading: preferencesLoading } = useUserPreferences();
  const { library: agentLibrary, loading: agentLibraryLoading } = useAgentLibrary();
  ...
  const sandboxName = SANDBOX_OPTIONS.find((s) => s.id === sandboxType)?.name ?? sandboxType;
  ...
  const isRepoSelectionComplete = mode !== "repo" || (selectedOwner && selectedRepo);
  ...
  const controlsDisabled = isLoading || preferencesLoading || agentLibraryLoading;
  ...
  const handleSubmit = () => {
    ...
    if (shouldLoadVercelProjects) {
      if (repoProjectsError || !repoProjects) {
        ...
        vercelProject =
          repoProjects.projects.find((project) => project.projectId === vercelProjectChoice) ??
          null;
      } else {
        return;
      }
    ...
    const agentName =
      selectedAgentValue === "__default__"
        ? undefined
        : selectedAgentValue === "__none__"
          ? null
          : selectedAgentValue;
      ...
      <div className="flex flex-col gap-4">
        ...
        {mode === "repo" && (
          <div className="flex flex-col gap-3">
            ...
            />
            {selectedOwner && selectedRepo && !githubConnectionLoading && !reconnectRequired && (
        ...
        <div className="grid gap-2">
          <Select
            ...
            onValueChange={setSelectedAgentValue}
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="__default__">
                {agentLibrary?.defaultAgentName
                  ? `Default agent (${agentLibrary.defaultAgentName})`
                  : "Default agent"}
              </SelectItem>
              <SelectItem value="__none__">No custom agent</SelectItem>
              {(agentLibrary?.agents ?? []).map((agent) => (
          ...
          <p className="text-xs text-muted-foreground">
            Agents configure reusable tools, skills, repos, model, and prompt.
          </p>
        </div>
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate text-xs text-muted-foreground">
              {effectiveAutoCommitPush ? (
                <>
                  Auto commit <span className="font-medium text-foreground/80">on</span>
                  {effectiveAutoCreatePr && (
                    <>
                      {" · "}Auto PR <span className="font-medium text-foreground/80">on</span>
                    </>
                  )}
                </>
              ) : (
            ...
            <div className="border-t border-border/50 dark:border-white/[0.06]">
              ...
              {effectiveAutoCommitPush && (
                <div className="flex items-center justify-between gap-4 border-t border-border/30 px-3 py-2 pl-6 dark:border-white/[0.04]">
                  <p className="text-sm text-muted-foreground">Create pull request</p>
                  <Switch
        ...
        <p className="text-center text-xs text-muted-foreground">
          Using {sandboxName} sandbox <span className="text-muted-foreground/60">&middot;</span>{" "}
          <Link
