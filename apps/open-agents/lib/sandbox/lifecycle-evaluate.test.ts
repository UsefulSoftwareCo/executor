...
const spies = {
  ...
  getEveChatStreamingStatuses: mock(async (chatIds: readonly string[]) => {
    return new Map(
      chatIds.map((chatId) => [chatId, streamingChatIds.has(chatId)]),
    );
  }),
  updateSession: mock(
...
mock.module("@/lib/db/sessions", () => ({
  ...
  getEveChatStreamingStatuses: spies.getEveChatStreamingStatuses,
...
describe("evaluateSandboxLifecycle", () => {
  test("skips hibernation whenever any chat still has an Eve stream", async () => {
    chatsInSession = [{ id: "chat-1" }];
    streamingChatIds = new Set(["chat-1"]);
  ...
  test("rechecks for Eve streams before stopping and restores active lifecycle state", async () => {
