...
function toGitErrorMessage(result: { stderr?: string; stdout?: string }): string {
...
async function hasCommitsToPush(params: {
  ...
  return result.stdout.includes("needs-push") || result.stdout.trim().length > 0;
}
...
async function pushBranchToRemote(params: {
  ...
  try {
    const pushResult = await withTemporaryGitHubAuth(params.sandbox, syncToken.token, () =>
      params.sandbox.exec(
...
async function syncSandboxBeforeCommit(params: {
  sandbox: Awaited<ReturnType<typeof connectSandbox>>;
  branch: string;
  access: Extract<RepoAccessResult, { ok: true }>;
}): Promise<void> {
  if (params.access.authStrategy === "user-token") {
    await withTemporaryGitHubAuth(params.sandbox, params.access.token, () =>
      syncToRemotePreservingChanges(params.sandbox, params.branch),
    );
    return;
  }

  const syncToken = await mintInstallationToken({
    installationId: params.access.installationId,
    repositoryIds: [params.access.repositoryId],
    permissions: { contents: "read" },
  });

  try {
    await withTemporaryGitHubAuth(params.sandbox, syncToken.token, () =>
      syncToRemotePreservingChanges(params.sandbox, params.branch),
    );
  } finally {
    await revokeInstallationToken(syncToken.token);
...
export async function commitChanges(params: {
  ...
  const { sessionId, sessionTitle, baseBranch, branchName, commitTitle, commitBody } = params;
  ...
  const branchResult = await sandbox.exec("git symbolic-ref --short HEAD", cwd, 10000);
  ...
  const isDetachedOrOnBase = resolvedBranch === baseBranch || looksLikeCommitHash(resolvedBranch);

  if (isDetachedOrOnBase) {
    const generatedBranch = generateBranchName(session.user.username, session.user.name);
    ...
    const checkoutResult = await sandbox.exec(`git checkout -b ${generatedBranch}`, cwd, 10000);
