...
interface ListGhCliRepositoriesOptions {
  token: string;
  owner: string;
  query?: string;
  limit?: number;
...
export async function listGhCliRepositories({
  token,
  owner,
  query,
  limit,
}: ListGhCliRepositoriesOptions): Promise<InstallationRepository[]> {
  const ownerFilter = owner.trim().toLowerCase();
  const queryFilter = query?.trim().toLowerCase();
  const normalizedLimit = normalizeLimit(limit);
  const matchedRepos: z.infer<typeof installationRepoSchema>[] = [];
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    if (!response.ok) {
      const body = await response.text();
      throw new Error(
        `Failed to fetch gh CLI repositories: ${response.status} ${body}`,
      );
    }

    const json = await response.json();
    const parsed = z.array(installationRepoSchema).safeParse(json);
    if (!parsed.success) {
    ...
    const pageMatches = parsed.data.filter((repo) => {
      const matchesOwner = repo.owner.login.toLowerCase() === ownerFilter;
      const matchesQuery = queryFilter
        ? repo.name.toLowerCase().includes(queryFilter) ||
