...
export type RepoAccessResult =
  | {
      ...
      authStrategy: "installation" | "user-token" | "public";
      token?: string;
    }
...
function isPublicRepoAccessEnabled(): boolean {
  return process.env.OPEN_AGENTS_ALLOW_PUBLIC_REPO_SESSIONS === "true";
}

async function getPublicRepoAccess(owner: string, repo: string): Promise<RepoAccessResult | null> {
  ...
  if (!response.ok) {
    throw new Error(`GitHub public repository lookup failed with status ${response.status}`);
  }
...
export async function verifyRepoAccess(params: {
  ...
  const [userOctokit, userToken] = await Promise.all([
    getUserOctokit(userId),
    getUserGitHubToken(userId),
  ]);
  if (!userOctokit || !userToken) {
    if (requiredUserPermission === "read" && isPublicRepoAccessEnabled()) {
      const publicAccess = await getPublicRepoAccess(owner, repo);
      if (publicAccess) {
        return publicAccess;
      }
    }
  ...
  if (!installation) {
    if (isGhCliAuthEnabled() || isGlobalGitHubTokenEnabled()) {
      return {
        ok: true,
        installationId: 0,
        repositoryId,
        defaultBranch,
        authStrategy: "user-token",
        token: userToken,
      };
    }
