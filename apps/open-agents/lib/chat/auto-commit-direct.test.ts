/* oxlint-disable executor/no-try-catch-or-throw, executor/no-error-constructor -- boundary: test double models rejected sandbox operations */
...
let apiCommitResult: { ok: true; commitSha: string } | { ok: false; error: string } = {
  ok: true,
...
mock.module("@open-agents/model-settings", () => ({
...
mock.module("@open-agents/sandbox", () => ({
  ...
  getChangedFiles: async () => changedFiles.map(({ path, status }) => ({ path, status })),
...
mock.module("@/lib/github/commit", () => ({
  ...
  buildCommitMessageWithCoAuthor: (message: string, coAuthor?: { name: string; email: string }) =>
    coAuthor ? `${message}\n\nCo-Authored-By: ${coAuthor.name} <${coAuthor.email}>` : message,
}));
  ...
  return {
    sandbox: {
      ...
      exec: async (command: string) => ({
        success: true,
        stdout: command.startsWith("git rev-list --count") ? "0\n" : "",
      }),
    } as never,
    userId: "user-1",
    sessionId: "session-1",
