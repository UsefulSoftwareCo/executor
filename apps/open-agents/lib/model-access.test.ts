...
describe("model access gating", () => {
  test("filters Claude Opus base models for managed trial users", () => {
    const result = filterModelsForSession(
      [{ id: "anthropic/claude-opus-4.6" }, { id: "anthropic/claude-haiku-4.5" }],
      managedTrialSession,
      requestUrl,
    );
  ...
  test("sanitizes managed trial preferences without mutating the database shape", () => {
    ...
    expect(result).toMatchObject({
      ...
      defaultSubagentModelId: "anthropic/claude-sonnet-4.6",
      modelVariants: [],
  ...
  test("leaves Vercel users unchanged", () => {
    const result = sanitizeUserPreferencesForSession(basePreferences, vercelSession, requestUrl);
