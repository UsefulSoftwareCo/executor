...
import { normalizeGlobalSkillRefs, type GlobalSkillRef } from "@/lib/skills/global-skill-refs";
...
function normalizeSandboxType(value: unknown): SandboxType {
  if (typeof value === "string" && VALID_SANDBOX_TYPES.includes(value as SandboxType)) {
...
function normalizeDiffMode(value: unknown): DiffMode {
  if (typeof value === "string" && VALID_DIFF_MODES.includes(value as DiffMode)) {
...
export function toUserPreferencesData(
  ...
  const parsedModelVariants = modelVariantsSchema.safeParse(row?.modelVariants ?? []);
  const rawDefaultAgentName = row ? row.defaultAgentName : DEFAULT_PREFERENCES.defaultAgentName;
  ...
  return {
    ...
    alertSoundEnabled: row?.alertSoundEnabled ?? DEFAULT_PREFERENCES.alertSoundEnabled,
    publicUsageEnabled: row?.publicUsageEnabled ?? DEFAULT_PREFERENCES.publicUsageEnabled,
    globalSkillRefs: normalizeGlobalSkillRefs(row?.globalSkillRefs),
    defaultAgentName:
      typeof rawDefaultAgentName === "string" && rawDefaultAgentName.trim()
        ? rawDefaultAgentName
        : null,
...
export async function updateUserPreferences(
  ...
  const [created] = await db
    ...
    .values({
      id: nanoid(),
      userId,
      defaultModelId: updates.defaultModelId ?? DEFAULT_PREFERENCES.defaultModelId,
      defaultSubagentModelId: updates.defaultSubagentModelId ?? null,
      defaultSandboxType: updates.defaultSandboxType ?? DEFAULT_PREFERENCES.defaultSandboxType,
      defaultDiffMode: updates.defaultDiffMode ?? DEFAULT_PREFERENCES.defaultDiffMode,
      autoCommitPush: updates.autoCommitPush ?? DEFAULT_PREFERENCES.autoCommitPush,
      ...
      alertSoundEnabled: updates.alertSoundEnabled ?? DEFAULT_PREFERENCES.alertSoundEnabled,
      publicUsageEnabled: updates.publicUsageEnabled ?? DEFAULT_PREFERENCES.publicUsageEnabled,
      globalSkillRefs: updates.globalSkillRefs ?? DEFAULT_PREFERENCES.globalSkillRefs,
      defaultAgentName: updates.defaultAgentName ?? DEFAULT_PREFERENCES.defaultAgentName,
      modelVariants: updates.modelVariants ?? DEFAULT_PREFERENCES.modelVariants,
      enabledModelIds: updates.enabledModelIds ?? DEFAULT_PREFERENCES.enabledModelIds,
    })
    .returning();
