...
export function useSessions(options?: {
  ...
  const sessionsEndpoint = includeArchived ? "/api/sessions" : "/api/sessions?status=active";
  ...
  const { data, error, isLoading, mutate } = useSWR<SessionsResponse>(
    ...
    () => fetcher<SessionsResponse>(sessionsEndpoint),
    {
      ...
      refreshInterval: (latestData) => {
        ...
        const hasStreamingSession = latestData?.sessions.some((s) => s.hasStreaming);
  ...
  const archiveSession = useCallback(
    async (sessionId: string) => {
      const previousData = cloneSessionsResponse(data);
      const sessionToArchive = previousData?.sessions.find((session) => session.id === sessionId);
      ...
      await mutate(
        (current) => {
          ...
          const nextArchivedCount =
            hadSession && !wasArchived ? (source.archivedCount ?? 0) + 1 : source.archivedCount;

          if (!includeArchived) {
            return {
              ...source,
              sessions: source.sessions.filter((session) => session.id !== sessionId),
  ...
  const unarchiveSession = useCallback(
    async (sessionId: string) => {
      const previousData = cloneSessionsResponse(data);
      const nextArchivedCount = Math.max((previousData?.archivedCount ?? 0) - 1, 0);
