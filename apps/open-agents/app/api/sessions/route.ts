...
import { getVercelProjectLinkByRepo, upsertVercelProjectLink } from "@/lib/db/vercel-project-links";
...
import { isVercelInvalidTokenError, listMatchingVercelProjects } from "@/lib/vercel/projects";
import { getUserVercelToken } from "@/lib/vercel/token";
import { vercelProjectSelectionSchema, type VercelProjectSelection } from "@/lib/vercel/types";
...
async function resolveSessionTitle(input: CreateSessionRequest, userId: string): Promise<string> {
...
export async function GET(req: Request) {
  ...
  if (statusParam === "archived") {
    ...
    if (searchParams.get("limit") !== null && rawLimit === null) {
      return Response.json({ error: "Invalid archived limit" }, { status: 400 });
    }

    if (searchParams.get("offset") !== null && rawOffset === null) {
      return Response.json({ error: "Invalid archived offset" }, { status: 400 });
    }
...
export async function POST(req: Request) {
  ...
  if (isTrialUser) {
    ...
    if (existingSessionCount >= MANAGED_TEMPLATE_TRIAL_SESSION_LIMIT) {
      return Response.json({ error: MANAGED_TEMPLATE_TRIAL_SESSION_LIMIT_ERROR }, { status: 403 });
    }
  }
  ...
  if (isTrialUser && (body.repoOwner || body.repoName || body.cloneUrl)) {
    return Response.json({ error: MANAGED_TEMPLATE_TRIAL_GITHUB_SESSION_ERROR }, { status: 403 });
  }
  ...
  if (body.autoCommitPush !== undefined && typeof body.autoCommitPush !== "boolean") {
    return Response.json({ error: "Invalid autoCommitPush value" }, { status: 400 });
  }

  if (body.autoCreatePr !== undefined && typeof body.autoCreatePr !== "boolean") {
    return Response.json({ error: "Invalid autoCreatePr value" }, { status: 400 });
  }

  if (
    body.repoOwner !== undefined &&
    (typeof body.repoOwner !== "string" || !isValidGitHubRepoOwner(body.repoOwner))
  ) {
    return Response.json({ error: "Invalid repository owner" }, { status: 400 });
  }
  ...
  if (body.vercelProject === null) {
    ...
    const parsedProject = vercelProjectSelectionSchema.safeParse(body.vercelProject);
    if (!parsedProject.success) {
      return Response.json({ error: "Invalid Vercel project" }, { status: 400 });
    }
  ...
  try {
    ...
    if (hasRepo && repoOwner && repoName) {
      if (explicitVercelProject) {
        ...
        if (!matchedProject) {
          return Response.json(
            {
              error: "Selected Vercel project no longer matches this repository",
            },
            { status: 400 },
          );
        }
    ...
    const [title, rawPreferences] = await Promise.all([titlePromise, preferencesPromise]);
    const preferences = sanitizeUserPreferencesForSession(rawPreferences, session, req.url);
    const explicitAgentName =
      body.agentName === undefined || body.agentName === null ? null : body.agentName.trim();
    const requestedAgentName =
      body.agentName === undefined ? preferences.defaultAgentName : explicitAgentName;
    const agent = requestedAgentName
      ? await getAgentDefinition(requestedAgentName, session.user.id)
      : null;
    if (requestedAgentName && !agent && body.agentName !== undefined) {
      return Response.json({ error: "Selected agent not found" }, { status: 400 });
    }
    const workspaceRepos =
      !repoOwner && !repoName && !cloneUrl && agent?.repos.length ? agent.repos : [];
    const effectiveAutoCommitPush = autoCommitPush ?? preferences.autoCommitPush;
    const effectiveAutoCreatePr = autoCreatePr ?? preferences.autoCreatePr;
    const initialChatId = nanoid();
    const initialModelId = agent?.model ?? preferences.defaultModelId;
    ...
    console.info("[sessions.create] created", {
      sessionId: result.session.id,
      chatId: initialChatId,
      requestedAgentName,
      resolvedAgentName: agent?.slug ?? null,
      modelId: initialModelId,
      sandboxType,
    });
