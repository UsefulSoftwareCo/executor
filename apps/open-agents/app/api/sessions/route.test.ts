...
mock.module("@/lib/agents/repository", () => ({
  getAgentDefinition: async (agentName: string) =>
    agentDefinitions.get(agentName) ?? null,
}));
...
describe("/api/sessions POST vercel project linking", () => {
  ...
  test("ignores a stale stored default agent when default is selected", async () => {
    const { POST } = await routeModulePromise;
  ...
  test("rejects an explicit missing agent selection", async () => {
    const { POST } = await routeModulePromise;

    const response = await POST(createJsonRequest({ agentName: "missing" }));
    const body = (await response.json()) as { error: string };

    expect(response.status).toBe(400);
    expect(body.error).toBe("Selected agent not found");
    expect(createCalls).toHaveLength(0);
  });

  test("resolves a valid stored default agent", async () => {
    const { POST } = await routeModulePromise;

    defaultAgentName = "agent-one";
    agentDefinitions.set("agent-one", {
      slug: "agent-one",
