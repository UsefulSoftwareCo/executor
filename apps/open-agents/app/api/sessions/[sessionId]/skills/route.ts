...
import { filterSkillsForAgent, resolveSessionAgent } from "@/lib/agents/session-agent";
...
function toSkillSuggestions(skills: Awaited<ReturnType<typeof discoverSkills>>): SkillSuggestion[] {
  return skills
...
export async function GET(req: Request, context: RouteContext) {
  ...
  const agent = await resolveSessionAgent(sessionRecord.agentName, sessionRecord.userId);

  if (!refresh) {
    const cachedSkills = await getCachedSkills(sessionId, sandboxState);
    if (cachedSkills !== null) {
      return Response.json({
        skills: toSkillSuggestions(filterSkillsForAgent(cachedSkills, agent)),
      });
    }
  }
  ...
  try {
    ...
    const response: SkillsResponse = {
      skills: toSkillSuggestions(filterSkillsForAgent(skills, agent)),
    };
    ...
    if (isSandboxUnavailableError(message)) {
      await updateSession(sessionId, {
        sandboxState: clearUnavailableSandboxState(sessionRecord.sandboxState, message),
