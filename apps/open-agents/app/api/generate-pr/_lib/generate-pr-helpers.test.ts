...
type EveEventRow = { event: HandleMessageStreamEvent; createdAt: Date };

let chats: ChatRecord[] = [];
let eventRowsByChatId: Record<string, EveEventRow[]> = {};

mock.module("@/lib/db/sessions", () => ({
  getChatsBySessionId: async () => chats,
  getEveChatEventRows: async (chatId: string) => eventRowsByChatId[chatId] ?? [],
}));
