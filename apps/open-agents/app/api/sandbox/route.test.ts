...
import { DEFAULT_SANDBOX_TIMEOUT_MS, DEFAULT_SANDBOX_VCPUS } from "@/lib/sandbox/config";
...
mock.module("@/lib/vercel/projects", () => ({
  buildDevelopmentDotenvFromVercelProject: async (input: Record<string, unknown>) => {
...
mock.module("@/lib/db/sessions", () => ({
  ...
  getEveChatStreamingStatuses: async () => new Map<string, boolean>(),
...
mock.module("@/lib/skills/global-skill-installer", () => ({
  installGlobalSkills: async (params: {
    sandbox: {
      ...
      exec: (command: string, cwd: string, timeoutMs: number) => Promise<unknown>;
    };
    globalSkillRefs: Array<{ source: string; skillName: string }>;
  }) => {
...
mock.module("@/lib/agents/session-agent", () => ({
  resolveSessionAgent: async () => null,
  installAgentLocalSkills: async () => undefined,
}));

mock.module("@open-agents/sandbox", () => ({
  connectSandbox: async (state: ConnectConfig["state"], options?: ConnectConfig["options"]) => {
...
describe("/api/sandbox lifecycle kicks", () => {
  ...
  test("repo sandboxes use a setup-only installation token instead of embedding it", async () => {
    ...
    sessionRecord.cloneUrl = "https://github.com/acme/private-repo";
  ...
  test("workspace sandboxes clone persisted session repositories", async () => {
    const { POST } = await routeModulePromise;
    ...
    expect(connectConfigs[0]).toMatchObject({
      state: {
        ...
        sources: [
          {
            repo: "https://github.com/GoAugment/augment-web",
            branch: "staging",
            directory: "augment-web",
          },
          {
            repo: "https://github.com/GoAugment/augment-services",
            branch: "main",
            directory: "augment-services",
          },
          {
            repo: "https://github.com/GoAugment/augment-voice",
            branch: "main",
            directory: "augment-voice",
          },
        ],
  ...
  test("rejects repo URLs that only contain github.com in the path", async () => {
    ...
    sessionRecord.cloneUrl = "https://attacker.example/github.com/acme/private-repo";
  ...
  test("new sandboxes install global skills", async () => {
    ...
    sessionRecord.globalSkillRefs = [{ source: "vercel/ai", skillName: "ai-sdk" }];
