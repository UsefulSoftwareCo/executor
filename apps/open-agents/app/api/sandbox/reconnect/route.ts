...
export async function GET(req: Request): Promise<Response> {
  ...
  if (!hasRuntimeSandboxState(sessionRecord.sandboxState)) {
    console.log(
      `[Reconnect] session=${sessionId} status=no_sandbox hasResumableSandbox=${hasResumableSandbox} runtimeState=false`,
    );
  ...
  try {
    ...
    console.log(
      `[Reconnect] session=${sessionId} status=connected hasResumableSandbox=${hasResumableSandbox} expiresAt=${sandbox.expiresAt ?? "null"}`,
    );
    return Response.json({
      status: "connected",
      hasResumableSandbox,
    ...
    console.error(
      `[Reconnect] session=${sessionId} status=expired hasResumableSandbox=${hasResumeStateAfterFailure} error=${message}`,
    );
    return Response.json({
      status: "expired",
      hasResumableSandbox: hasResumeStateAfterFailure,
      lifecycle: {
        serverTime: Date.now(),
        state: "hibernated",
        lastActivityAt: null,
