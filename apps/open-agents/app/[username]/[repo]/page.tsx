...
async function fetchRepoInfo(
  ...
  if (!response.ok) {
    const message = `[repo-page] GitHub API returned ${response.status} for /repos/${owner}/${repo}`;
...
export default async function RepoPage({ params }: RepoPageProps) {
  ...
  const savedVercelProjectPromise = getVercelProjectLinkByRepo(session.user.id, username, repo);
  ...
  const agent = preferences.defaultAgentName
    ? await getAgentDefinition(preferences.defaultAgentName, session.user.id)
    : null;
  ...
  const result = await createSessionWithInitialChat({
    session: {
      ...
      autoCreatePrOverride: preferences.autoCommitPush ? preferences.autoCreatePr : false,
    ...
    initialChat: {
      id: nanoid(),
      title: "New chat",
      modelId: agent?.model ?? preferences.defaultModelId,
    },
  });
