...
export function SessionChatContent({
  ...
  // Sync hasPendingResponse with the runtime status.
  ...
  useEffect(() => {
    ...
    // previous turn. Without this guard the
  ...
  const showThinkingIndicator = useMemo(() => {
    ...
    // runtime status hasn't caught up yet), always show
  ...
  useEffect(() => {
    ...
    const fileModifyingTools = ["tool-write_file"];
  ...
  const { pendingQuestion, questionRequestId } = useMemo(() => {
  ...
  const handleQuestionSubmit = useCallback(
    (answer: { optionId?: string; text?: string }) => {
      if (questionRequestId) {
        void addInputResponse({
          requestId: questionRequestId,
          ...answer,
        });
      }
    },
    [questionRequestId, addInputResponse],
  );

  const handleQuestionCancel = useCallback(() => {
    if (questionRequestId) {
      void sendMessage({ text: "skip this question" });
    }
  }, [questionRequestId, sendMessage]);
                    ...
                    >
                      ...
                      >
                        ...
                        <SandboxInputOverlay
                          ...
                          pausePending={isArchivePausePending}
                        />
