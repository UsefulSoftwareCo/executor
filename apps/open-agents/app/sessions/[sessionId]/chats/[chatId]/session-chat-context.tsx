...
type SessionChatContextValue = {
  ...
  /** The initial Eve event log used for reducer hydration */
  initialEveEvents: HandleMessageStreamEvent[];
  /** The initial Eve session cursor used for follow-up turns */
  ...
  /** Whether the session has a preserved sandbox handle available for resume */
  hasResumableSandbox: boolean;
  ...
  /** Clear a transient chat error and reset the Eve runtime */
...
export function SessionChatProvider({
  ...
  const [hasResumableSandboxState, setHasResumableSandboxState] =
    useState<boolean>(
  ...
  const attemptReconnection =
    useCallback(async (): Promise<ReconnectionStatus> => {
      ...
      try {
        ...
        setHasResumableSandboxState(data.hasResumableSandbox);
        ...
        if (data.status === "no_sandbox" || data.status === "expired") {
          ...
          setSessionRecord((prev) => ({
            ...prev,
            sandboxState: data.hasResumableSandbox
