...
type SensitiveToolName = "read_file" | "write_file";
type NestedSensitiveToolName = SensitiveToolName | "agent";
  ...
  for (const message of messages) {
    ...
    for (const part of content) {
      ...
      if (part.toolName === "agent") {
        nestedSensitiveToolCalls.set(part.toolCallId, "agent");
      }
    }
  }
        ...
        if (
          ...
          return {
            ...
            output: sanitizeToolOutput(
              nestedSensitiveToolCalls.get(part.toolCallId) ?? "read_file",
              part.output,
            ),
          };
        }
...
function sanitizeMessagePart(
  ...
  switch (part.type) {
    case "dynamic-tool": {
      ...
      if (part.toolName === "agent" && part.state === "output-available") {
        return {
          ...part,
    ...
    case "tool-write_file":
      ...
      return {
        ...part,
        input: sanitizeToolCallInput("write_file", part.input) as typeof part.input,
      } as WebAgentUIMessagePart;
    case "tool-agent":
