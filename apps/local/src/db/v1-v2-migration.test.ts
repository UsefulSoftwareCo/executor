...
const seedDuplicateStripeScope = async (dbPath: string, scopeId: string) => {
  const client = await openLocalLibsql(dbPath);
  const now = Date.now();
  ...
  await executeSql(
    ...
    [
      `${scopeId}/onepassword`,
      "config",
      JSON.stringify({ vaultId: "vault-456" }),
      "blob-row-duplicate-scope",
      JSON.stringify([`${scopeId}/onepassword`, "config"]),
    ],
  );
...
describe("local v1 -> v2 migration", () => {
  ...
  it("preserves distinct local v1 scopes as distinct v2 tenants", async () => {
    const firstScopeId = "executor-workspace-abcd1234";
    const secondScopeId = "executor-fork-ef567890";
    const dataDir = join(workDir, "data");
    const dbPath = join(dataDir, "data.db");
    mkdirSync(dataDir, { recursive: true });
    await seedV1Db(dbPath, firstScopeId);
    await seedDuplicateStripeScope(dbPath, secondScopeId);
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          [firstScopeId]: { "stripe-key": "sk_test_first" },
          [secondScopeId]: { "stripe-key": "sk_test_second" },
        },
        null,
        2,
      ),
    ...
    const result = await migrateLocalV1ToV2IfNeeded({
      ...
      tables: collectTables(),
      namespace: "executor_local",
      tenantId: firstScopeId,
    });

    expect(result.migrated).toBe(true);
    expect(result.report).toMatchObject({ integrations: 2, connections: 2, secretOps: 2 });

    const client = await openLocalLibsql(dbPath);
    const integrations = await client.execute(
      "SELECT tenant, slug, plugin_id FROM integration ORDER BY tenant, slug",
    );
    expect(integrations.rows).toEqual([
      { tenant: secondScopeId, slug: "stripe_api", plugin_id: "openapi" },
      { tenant: firstScopeId, slug: "stripe_api", plugin_id: "openapi" },
    ]);

    const connections = await client.execute(
      "SELECT tenant, owner, subject, integration, name, provider FROM connection ORDER BY tenant, integration, name",
    );
    expect(connections.rows).toEqual([
      {
        tenant: secondScopeId,
        owner: "org",
        subject: "",
        integration: "stripe_api",
        name: "stripeKey",
    ...
    const tools = await client.execute(
      "SELECT tenant, owner, subject, integration, connection, name FROM tool ORDER BY tenant, integration, connection, name",
    );
    expect(tools.rows).toEqual([
      {
        tenant: secondScopeId,
    ...
    const auth = decodeAuthFile(readFileSync(join(authDir, "auth.json"), "utf-8"));
    expect(auth[migratedItemId(firstScopeId, "stripe-key")]).toBe("sk_test_first");
    expect(auth[migratedItemId(secondScopeId, "stripe-key")]).toBe("sk_test_second");
  });
