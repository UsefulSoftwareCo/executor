  ...
  it.effect(scenarios.codeFormatting.title, (context) =>
    withCase(
      ...
      Effect.gen(function* () {
        ...
        yield* browser.use("Open retained source", (page) =>
          page.goto(`/org/${actors.organization.slug}/apps/${app.id}?view=deployments`),
        );
        const read = () =>
        ...
        for (const file of snippets) {
          ...
          } else if (file.path === "unfinished.ts" || file.path === "command.sh")
          ...
          if (file.path === "data.json" || file.path === "format-demo.ts")
            yield* browser.checkpoint(`Formatted ${file.path}`);
        }
