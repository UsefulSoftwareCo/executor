...
// org B, follows an org-A-scoped link. The request must resolve in org A and
// stop at the admin-required state there — never enter a connection flow in B.
//
      ...
      Effect.gen(function* () {
        ...
        yield* browser.session(forBrowser(inOrgB), async ({ page, step }) => {
          await step("Follow the org-A connect link with an org-B session", async () => {
            ...
            // A plain member cannot add a connection, so the deep link stays on
            // its honest refusal state. The org-A prefix still proves the link
            // resolved against the SENDING workspace rather than session org B.
            await page
              .getByText("Workspace admin required", { exact: true })
              .waitFor({ state: "visible", timeout: 30_000 });
            expect(
              new URL(page.url()).pathname.split("/").filter(Boolean)[0],
              "the refusal is scoped to the SENDING org, not the session default",
            ).toBe(orgA.slug);
            expect(
              new URL(page.url()).pathname,
              "the member never enters the integration add-account route",
            ).toBe(`/${orgA.slug}/connect/${slug}`);
          });
          ...
          // so this page exists and renders its own admin-required state too.
          await step("Org B — the session's own default — also offers no add action", async () => {
            ...
            expect(
              ...
              "the member cannot add in the session-default org either",
            ).toBe(0);
          });
        });
      }),
