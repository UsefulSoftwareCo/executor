...
export const verifyAdmin = (baseUrl: string, identity: Identity): Effect.Effect<Identity> =>
  Effect.promise(async () => {
    ...
    const secret = setup?.secret ?? (Option.isSome(decodeChallenge(raw)) ? undefined : undefined);
...
export const verifyAdminInBrowser = async (page: Page, secret?: string): Promise<void> => {
  await page.getByRole("heading", { name: "Unlock organization keys" }).waitFor();
  ...
  await page
    .getByRole("heading", { name: "Unlock organization keys" })
