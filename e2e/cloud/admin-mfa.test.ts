...
scenario(
  ...
  Effect.gen(function* () {
    ...
    yield* browser.session(identity, async ({ page, step }) => {
      await step("Use the workspace and personal keys without enrolling", async () => {
        await page.goto(`/${orgSelectorOf(identity)}/api-keys`);
        await page.getByRole("heading", { name: "Personal keys", exact: true }).waitFor();
        await page.getByRole("heading", { name: "Unlock administration" }).waitFor();
        expect(await page.getByRole("button", { name: "New org key" }).count()).toBe(0);
        await page.getByRole("link", { name: "Back to workspace" }).click();
        await page.waitForURL((url) => url.pathname === `/${orgSelectorOf(identity)}`);
        await page
          .getByRole("heading", { name: "Unlock administration" })
          .waitFor({ state: "detached" });
        await page.getByRole("link", { name: "Unlock administration", exact: true }).waitFor();
        await page.getByRole("heading", { name: "Integrations", exact: true }).waitFor();
        expect(await page.getByRole("heading", { name: "Unlock administration" }).count()).toBe(0);
      });
      ...
      await step("Start setup and cancel it", async () => {
        ...
        await page.getByRole("link", { name: "Back to workspace" }).click();
        await page.getByRole("link", { name: "Unlock administration", exact: true }).click();
      });
      let secret = "";
      ...
      await step("Enter the current code and open admin settings", async () => {
        ...
        expect(proof).toMatchObject({ httpOnly: true, secure: true, sameSite: "Lax" });
      ...
      await step("Lock administration and keep using the workspace", async () => {
        await page.goto(`/${orgSelectorOf(identity)}`);
        await page.getByRole("button", { name: "Lock administration", exact: true }).click();
        await page.getByRole("link", { name: "Unlock administration", exact: true }).waitFor();
        await page.getByRole("heading", { name: "Integrations", exact: true }).waitFor();
        const headers = { "x-executor-organization": orgSelectorOf(identity) };
        expect((await page.request.get("/api/admin/users", { headers })).status()).toBe(403);
        expect((await page.request.get("/api/integrations", { headers })).status()).toBe(200);
      });
    });
  }),
);
