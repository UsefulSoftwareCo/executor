...
//
// The scenario deliberately leaves `remoteTransport` at its default ("auto"):
// most real connections run auto, and auto's streamable-http -> SSE fallback
// used to swallow the 401 (the SSE retry fails with an opaque error), reading
// an expired token as merely degraded. The fallback now propagates a
// definitive 401/403 as-is, so this asserts the DEFAULT path classifies
// expiry correctly.
